...
import { IV_BYTES, TAG_BYTES, open, seal } from './sealing.js'

const secret = 'bridge-secret-that-is-long-enough-32'
const other = 'a-different-secret-also-long-enough!'
const payload = JSON.stringify({ projectId: '01M240ERCRWWCN16Q5AHP1FZAQ', token: 'sharetokenabcdef0123456789' })
...
describe('seal and open', () => {
  ...
  it('round-trips the empty string to the empty string, and not to null', () => {
    const sealed = seal(secret, '')
    const opened = open(secret, sealed)
    expect(opened).toBe('')
  ...
  it('refuses a blob whose ciphertext body has one bit flipped', () => {
  ...
  it('refuses a blob whose IV has one bit flipped, addressed by byte offset zero', () => {
    const sealed = seal(secret, payload)
  ...
  it('refuses a blob shorter than IV_BYTES + TAG_BYTES rather than throwing', () => {
    const short = Buffer.alloc(IV_BYTES + TAG_BYTES - 1).toString('base64url')
    expect(() => open(secret, short)).not.toThrow()
    expect(open(secret, short)).toBeNull()
  })
  ...
  it.each([
    ...
    expect(() => open(secret, value)).not.toThrow()
