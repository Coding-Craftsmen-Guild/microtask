...
describe('FolderService.list', () => {
  it('lists nothing for a project that has no folders', async () => {
    ...
    await seed()
    await expect(service.list('microtask', PROJECT)).resolves.toEqual([])
  })
  ...
  it('lists folders in position order rather than the order they are stored in', async () => {
    ...
    const listed = await service.list('microtask', PROJECT)
    expect(listed.map((f) => f.name)).toEqual(['A', 'B', 'C'])
  })

  it('rejects an unknown project', async () => {
    await expect(build().service.list('microtask', ABSENT)).rejects.toThrow(NotFound)
  })
})
...
describe('FolderService.create', () => {
  ...
  it('creates a folder with no parent field, and takes no parent argument, because folders never nest', async () => {
    const { service, seed } = build()
    await seed()
    const created = await service.create('microtask', PROJECT, 'Flat')
    expect(Object.keys(created).sort()).toEqual(['createdAt', 'id', 'name', 'position', 'updatedAt'])
    expect(service.create).toHaveLength(3)
  })

  it('cleans the name it was given', async () => {
    const { service, seed } = build()
    await seed()
    const created = await service.create('microtask', PROJECT, '  Client   work  ')
  ...
  it('stamps the new folder and its project from the clock', async () => {
    ...
    await seed()
    const created = await service.create('microtask', PROJECT, 'Fresh')
  ...
  it('rejects an unknown project', async () => {
    await expect(build().service.create('microtask', ABSENT, 'Nope')).rejects.toThrow(NotFound)
  })
})
...
describe('FolderService.rename', () => {
  it('renames the folder and stamps its updatedAt', async () => {
    ...
    const renamed = await service.rename('microtask', PROJECT, 'f-a', 'New')
  ...
  it('cleans the name it was given', async () => {
    const { service, seed } = build()
    await seed({ folders: [folder('f-a', 'Old')] })
    const renamed = await service.rename('microtask', PROJECT, 'f-a', '  Client   work  ')
  ...
  it('leaves the folder where it was in the order', async () => {
    ...
    const renamed = await service.rename('microtask', PROJECT, 'f-b', 'Bee')
    expect(renamed.position).toBe(1)
    expect((await service.list('microtask', PROJECT)).map((f) => f.name)).toEqual(['A', 'Bee'])
  })

  it('renames twice in a row, so reading inside the lock has not wedged the queue', async () => {
    const { service, seed } = build()
    await seed({ folders: [folder('f-a', 'Old')] })
    await service.rename('microtask', PROJECT, 'f-a', 'Mid')
    const renamed = await service.rename('microtask', PROJECT, 'f-a', 'End')
  ...
  it('rejects an unknown folder', async () => {
    ...
    await expect(service.rename('microtask', PROJECT, 'f-zzz', 'New')).rejects.toThrow(NotFound)
  })

  it('rejects an empty name', async () => {
    const { service, seed } = build()
    await seed({ folders: [folder('f-a', 'A')] })
    await expect(service.rename('microtask', PROJECT, 'f-a', '  ')).rejects.toThrow(Invalid)
  })
})
...
describe('FolderService.remove', () => {
  it('removes the folder', async () => {
    const { service, seed } = build()
    await seed({ folders: [folder('f-a', 'A')] })
    await service.remove('microtask', PROJECT, 'f-a')
    await expect(service.list('microtask', PROJECT)).resolves.toEqual([])
  })
  ...
  it('closes the gap it leaves, so folder positions stay dense', async () => {
    ...
    await service.remove('microtask', PROJECT, 'f-b')
    const listed = await service.list('microtask', PROJECT)
  ...
  it('rejects an unknown folder rather than reporting success', async () => {
    const { service, seed } = build()
    await seed({ folders: [folder('f-a', 'A')] })
    await expect(service.remove('microtask', PROJECT, 'f-zzz')).rejects.toThrow(NotFound)
  })
})
...
describe('FolderService.reorder', () => {
  it('puts the folders in the order it was given, numbered from zero', async () => {
    ...
    const reordered = await service.reorder('microtask', PROJECT, ['f-c', 'f-a', 'f-b'])
    expect(reordered.map((f) => f.name)).toEqual(['C', 'A', 'B'])
    expect(positionsOf(reordered)).toEqual([0, 1, 2])
    expect((await service.list('microtask', PROJECT)).map((f) => f.name)).toEqual(['C', 'A', 'B'])
  })
  ...
  it('refuses an order that leaves a folder out', async () => {
    ...
    await expect(service.reorder('microtask', PROJECT, ['f-a'])).rejects.toThrow(Invalid)
  })

  it('refuses an order naming a folder the project does not have', async () => {
    const { service, seed } = build()
    await seed({ folders: [folder('f-a', 'A')] })
    await expect(service.reorder('microtask', PROJECT, ['f-a', 'f-zzz'])).rejects.toThrow(Invalid)
  })
  ...
  it('refuses an order that names the same folder twice', async () => {
    ...
    await expect(service.reorder('microtask', PROJECT, ['f-a', 'f-a'])).rejects.toThrow(Invalid)
  })
  ...
  it('leaves the stored order untouched when it refuses', async () => {
    ...
    await expect(service.reorder('microtask', PROJECT, ['f-b'])).rejects.toThrow(Invalid)
    expect((await service.list('microtask', PROJECT)).map((f) => f.name)).toEqual(['A', 'B'])
  })
})
...
describe('FolderService caps', () => {
  ...
  it('holds the cap under concurrent creates, because the count is read inside the lock', async () => {
    ...
    await expect(service.list('microtask', PROJECT)).resolves.toHaveLength(LIMITS.foldersPerProject)
  })
})
