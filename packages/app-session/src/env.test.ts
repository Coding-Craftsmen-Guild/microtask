...
describe('readEnv', () => {
  it('reads the three values an app needs from the object it was handed', () => {
...
describe('what an Edge instrumentation bundle pulls in', () => {
  const NODE_BUILTINS = new Set([...builtinModules, ...builtinModules.map((name) => `node:${name}`)])

  const specifiers = (text: string): string[] =>
    [...text.matchAll(/\b(?:from|import)\s*\(?\s*['"]([^'"]+)['"]/g)].map((match) => match[1] ?? '')

  it('imports no Node built-in, since next build compiles each app’s register into the Edge runtime too', () => {
    const text = readFileSync(fileURLToPath(new URL('./env.ts', import.meta.url)), 'utf8')
    expect(specifiers(text).filter((specifier) => NODE_BUILTINS.has(specifier))).toEqual([])
    expect(text).not.toMatch(/\bBuffer\s*\./)
  })
})

describe('exactly one file in this workspace reads process.env', () => {
  it('is this package’s env.ts, and the lint config lifts n/no-process-env for it alone', () => {
    const config = readFileSync(fileURLToPath(new URL('../eslint.config.js', import.meta.url)), 'utf8')
    expect(config).toContain("files: ['src/env.ts']")
