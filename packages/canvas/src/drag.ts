 ...
 *
 * Not a pointer event and not a rect. Turning one event into one of these is the step no test in this
 * repository can check: `docs/adr/0055-canvas-geometry-is-its-own-pure-package.md` records why, and it
 * is the same reason everything here is arithmetic over numbers. Deciding what to do with the answer
 * is a client component's own work, and so is the request it sends.
 */
...
/**
 * Everything answering one drag takes: where it ended, which feature is moving, and the plan and the
 * layout it is moving within.
 *
 ...
 * still in `rails` and still in `plan` while it is being dragged, and every number computed as though
 * it were not moving lands it one place past where the user dropped it.
 *
 * `rails` and `plan` are one plan seen twice — laid out, and as it is stored. Both are needed because a
 * rail's bars and a rail's features are not the same list, which is what {@link DropTarget} is about.
 */
...
function landingAmong(
  rest: readonly ScheduleFeature[],
  remaining: readonly FeatureBar[],
  gap: number,
): number {
  const ahead = remaining[gap]
  if (ahead === undefined) return rest.length
  const found = rest.findIndex((feature) => feature.id === ahead.id)
  return found === -1 ? rest.length : found
}

const railFeatures = (plan: CanvasPlan, epicId: string): readonly ScheduleFeature[] =>
  railsOf(plan).find((rail) => rail[0]?.epicId === epicId) ?? []
 ...
 * placement there to answer with.
 *
 * The rule is to land where the bar the drop fell in front of sits in the stored order, with the dragged
 * feature lifted out of that order first. Three things separate that from counting bars, and each of the
 * three was a number that looked right:
 *
 ...
 *
 * Past every remaining bar the answer is the length of that lifted-out order, which is last on the rail.
...
export function dropTargetFor(query: DropQuery): DropTarget | null {
  ...
  const rest = railFeatures(plan, rail.epicId).filter((feature) => feature.id !== featureId)
  return { epicId: rail.epicId, position: landingAmong(rest, remaining, gap) }
}
