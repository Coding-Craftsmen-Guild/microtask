...
describe('this app binds the shared session to mp_admin', () => {
  it('seals into mp_admin and into no other cookie', async () => {
    ;(await session()).sealAdmin('admin.1.sig', 600)
    expect([...store.keys()]).toEqual(['mp_admin'])
  })

  it('gives the cookie the bearer own lifetime, never a constant', async () => {
    ;(await session()).sealAdmin('admin.1.sig', 1234)
    expect(store.get('mp_admin')?.maxAge).toBe(1234)
  })
  ...
  it('does not read the Microtask cookie, even sealed under the same secret', async () => {
    ;(await session()).sealAdmin('admin.1.sig', 600)
    const sealed = store.get('mp_admin')
    if (sealed === undefined) throw new Error('nothing was sealed')
    store.clear()
    store.set('mt_admin', { ...sealed, name: 'mt_admin' })
    expect((await session()).admin()).toBeNull()
  })

  it('clears mp_admin and leaves the Microtask cookie alone', async () => {
    ;(await session()).sealAdmin('admin.1.sig', 600)
    const sealed = store.get('mp_admin')
    if (sealed === undefined) throw new Error('nothing was sealed')
    store.set('mt_admin', { ...sealed, name: 'mt_admin' })
    ;(await session()).clearAdmin()
    expect(store.get('mp_admin')?.maxAge).toBe(0)
    expect(store.get('mt_admin')?.maxAge).toBe(600)
  })
})
