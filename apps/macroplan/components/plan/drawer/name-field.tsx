 ...
 * the house answer rather than the minimal one. {@link FieldShell} draws it outside the `<label>`, which
 * is why this field labels by `htmlFor` and not by wrapping: a refusal inside the label would become
 * part of the input's accessible name, and the field would be called "Feature name No." while it was
 * being fixed. The shell is also what points `aria-describedby` at that sentence, so the refusal reaches
 * a reader who tabs back as a **description** rather than as part of the name.
 *
