...
// The URL a seat is handed out as, which is what reaches the clipboard: never the bare token.
const URL_TEXT = 'https://plans.example.com/s/a_manage_seats_tok1'
...
describe('copySeatUrl', () => {
  ...
  it('answers true when the clipboard took it, without falling back', async () => {
    ...
    expect(await copySeatUrl(field(), URL_TEXT)).toBe(true)
  ...
  it('answers false when both fail, rather than claiming a copy that never happened', async () => {
    clipboard(() => Promise.reject(new Error('denied')))
    execCommand(() => false)
    expect(await copySeatUrl(field(), URL_TEXT)).toBe(false)
  })
