 ...
 * a browser asks for `/favicon.ico` regardless and nothing else would answer: a 404 on every
 * page, and a console error in every session.
 *
 * A 308 to {@link LOGO_PATH} rather than a copy of the bytes under a second name, so there stays
 * exactly one logo file per app to replace. `Location` is relative, so this needs nothing off the
