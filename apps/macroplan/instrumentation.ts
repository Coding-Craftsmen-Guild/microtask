 ...
 * secrets.
 *
 * Measured in apps/microtask against the standalone server with `COOKIE_SECRET` unset. With no
 * hook at all it logs `Ready`, serves `/login` with a 200, and fails only when someone signs in.
 * Letting the refusal throw out of here was no better: Next logs `An error occurred while loading
 * instrumentation hook`, keeps the process up and answers every request with a 500 — and Docker
 * restarts a container on exit, never on unhealthy, so `restart: unless-stopped` did nothing and
 * the deploy stayed up until a human read the health status. Exiting non-zero is what the
 ...
 * here, so this file adds no Node built-in to the Edge bundle `next build` also compiles
 * `register` into (ADR 0012, ADR 0027). Where the runtime has no `exit` to call, the refusal is
