...
describe('apps/macroplan/Dockerfile (ADR 0026)', () => {
  ...
  it('prunes to macroplan alone, once, positionally', () => {
    expect(DOCKERFILE.match(/turbo prune \S+/g)).toEqual(['turbo prune macroplan'])
    expect(DOCKERFILE).toContain('turbo prune macroplan --docker')
  })

  it('ships the standalone server with .next/static and public beside its nested server.js', () => {
    expect(inRunner('COPY')).toEqual([
      '--from=builder /repo/apps/macroplan/.next/standalone ./',
      '--from=builder /repo/apps/macroplan/.next/static ./apps/macroplan/.next/static',
      '--from=builder /repo/apps/macroplan/public ./apps/macroplan/public',
    ])
    expect(inRunner('CMD')).toEqual(['["node", "apps/macroplan/server.js"]'])
  })
