...
import { LOGIN_PATH } from '@repo/app-session/next-path'
import { redirect } from 'next/navigation'
import { loginWith } from '../lib/api'
import { LOGIN_REFUSED, refusalFor, reportLoginRefusal, signInDestination, type SignInState } from '../lib/login'
...
/**
 * Exchanges the admin password for a bearer, seals it into `mp_admin`, and follows `?next=`.
 *
 * The same one `ADMIN_PASSWORD` Microtask signs in with, through the same product-agnostic
 * `/v1/auth/login`: there is one admin, and the two products are two sessions rather than two
 * accounts (ADR 0012, ADR 0014).
 *
 ...
 *
 * `mp_admin` lives exactly as long as the bearer: its `Max-Age` is the `expiresInSeconds` the API
...
/**
 * Ends the admin session in this browser: clears `mp_admin` and lands on `/login`.
 *
 ...
 * route (ADR 0012's amendment, ADR 0032). Clearing the cookie ends the session in the only
 * browser that held it; a copy of the bearer taken before sign-out stays valid until its own
 * expiry, which `ADMIN_TOKEN_TTL_SECONDS` bounds.
 *
 * Microtask's session is untouched: it is sealed under a different cookie name, so signing out of
 * one product leaves the other signed in (ADR 0014).
 */
