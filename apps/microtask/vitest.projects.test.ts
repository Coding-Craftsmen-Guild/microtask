 ...
 * yet — which is exactly the state this app was in, with three of them open. Each case below is a
 * path no file used, measured as matching **no** project before the includes were widened: a
 * component test with no JSX in it, an action test that renders, and a test at the app root that
 * renders. Every one of them would have run zero assertions and reported nothing.
 */
describe('and so is every shape a test file in this app can take', () => {
  it.each([
    ['a component test with no JSX', 'components/transfer/upload.test.ts'],
    ['a component test that renders', 'components/transfer/upload.test.tsx'],
    ['an action test', 'actions/transfer.test.ts'],
    ['an action test that renders', 'actions/transfer.test.tsx'],
    ['a lib test', 'lib/env.test.ts'],
    ['a lib test that renders', 'lib/problem.test.tsx'],
    ['a route handler test', 'app/api/export/route.test.ts'],
    ['a page test', 'app/(admin)/transfer/page.test.tsx'],
