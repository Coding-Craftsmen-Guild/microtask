...
| `/admin/projects/:projectId?tab=<tabId>`, the page rewriting its own URL to carry `?tab=` on every switch | `307` to `/p/:projectId/t/:projectId`, a legacy file being one task under the project’s own id; a `?tab=` rides through as a `?tab=`, naming an inner tab again, and a project holding no task of its own id falls back to the project page rather than 404ing (corrected 2026-09-21) | [0046](../adr/0046-legacy-admin-address-redirect.md), [0022](../adr/0022-hostname-continuity-gated-cutover.md) |
...
*project* is a new container of folders and tasks above it (ADR 0004). Legacy *data* maps the same
way, since **2026-09-21**: an old project becomes a Project holding exactly **one Task**, under the
project's own id and taking its name, whose tab strip is the old tabs (spec §7.6, corrected in
place). Until that date §7.6 said each old tab became a Task of its own holding one `General` tab,
which contradicted the sentence above it here — the legacy project *is* the task page — and is what
the product owner reported as broken. The UI rows below are audited against the task page; the data
rows against the API and the domain.
...
| R3 | `/admin/projects/:projectId` | REPRODUCED | **Closed by the import/export plan, Task 13; retargeted 2026-09-21.** The old address is answered rather than 404’d: a `307` to `/p/:projectId/t/:projectId`, carrying a legacy `?tab=` through as a `?tab=` — which names an inner **tab** again, §7.6 having been corrected so that a legacy file imports as one task whose tab strip is the legacy tabs. It therefore lands on the tab strip the legacy page showed, which is what that address opened, with or without a `?tab=`. A project holding no task of its own id — any not imported from legacy, and one imported as a *copy* — falls back to the project page rather than 404ing. The stale-`?tab=` case needs no guard here any more: the task page already falls back to its first tab, exactly as legacy did (feature 39). Admin bookmarks get the continuity `/share/` got, decided in [ADR 0046](../adr/0046-legacy-admin-address-redirect.md) alongside ADR 0022 and amended there. `app/admin/projects/[projectId]/route.ts` · `route.test` "answers the task page of the project’s own id, which is the tab strip the old page showed", "is answered 307 and never 308, both targets being chosen from data that can change", "carries the tab id onto the task page rather than consuming it into the path", "falls back for a project holding no task of its own id, which is any not imported from legacy", "maps every tab of fixture %i onto that one task page, as its ?tab="; the importer half of that mapping in `packages/microtask-domain/src/import/legacy.test.ts` "keeps the project id and every tab id of a real legacy file, which is what the R3 redirect maps"; the gate in `proxy.test` "sends a navigation with no mt_admin to /login, keeping the legacy address as the deep link"; the handler set in `api/handlers.test` "has exactly these three, so a fourth cannot appear without a decision" |
