...
**Totals.** Features: 24 reproduced, 42 changed (2 of them †), 0 dropped, 5 gaps. Routes: 1
...
| 15 | Inline title rename | REPRODUCED | Legacy's rules exactly — Enter commits, Escape reverts, blur commits, whitespace collapses, empty or unchanged restores with no request, never overwritten while focused, shows the server's stored name, spellcheck off — on an `<input>` rather than a `contenteditable`, for the project title and each task's name (plan Task 10). `task-tree/inline-name.tsx` · `inline-name.test` (all); `project-header.test` "renames this project from its title" |
...
| U36 | No realtime, no polling | REPRODUCED | On purpose (the table at the top; spec §3.1): the app opens no socket and sets no poll |
